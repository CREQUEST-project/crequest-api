...
def read_search_for_care_histories(
    ...
    count_statement = select(func.count(SearchForCareHistory.id)).where(
        SearchForCareHistory.user_id == user_id
    )
    count = session.exec(count_statement).one()
    db_search_for_care_histories = session.exec(
        select(SearchForCareHistory).where(SearchForCareHistory.user_id == user_id)
    ).all()

    return SearchForCareHistoryListOut(data=db_search_for_care_histories, count=count)


def read_search_for_care_history(
    ...
    if db_search_for_care_history is None:
        raise HTTPException(
            ...
            detail="Search for care history not found",
        )
