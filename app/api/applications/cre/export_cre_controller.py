...
from models.factors import CreResultSendEmail, Factors, MotifSearch
...
def export_excel(session: Session, data_in: MotifSearch):
    ...
    forward_factors_id = [
        match["factor_id"] for match in data_matches["forward_strand_matches"]
    ]
    reverse_factors_id = [
        match["factor_id"] for match in data_matches["reverse_strand_matches"]
    ]

    forward_factors = session.exec(
        select(Factors).where(Factors.ac.in_(forward_factors_id))
    ).all()
    reverse_factors = session.exec(
        select(Factors).where(Factors.ac.in_(reverse_factors_id))
    ).all()
